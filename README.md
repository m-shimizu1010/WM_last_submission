...
### プロファイリング結果（タスク: `xarm_lift`）
各手法を定量的に評価するためのプロファイリングを実施しました。結果は以下の通りです。
| 手法 (Method) | 更新パラメータ数 | 推論時間 (ms/step) | 学習時間 (ms/step) | 最大メモリ (MB) |
| :--- | :---: | :---: | :---: | :---: |
| **Full-FT** | 2,734,573 | 50.70 | 77.90 | 1260.9 |
| **Encoder-Frozen** | 2,656,929 | 54.21 | 77.36 | 1260.3 |
| **Dynamics-LoRA** | **2,083,174** | 65.92 | **76.94** | **1256.2** |

---

### 分析と考察

1.  **更新パラメータ数 (Trainable Parameters)**
    *   **Dynamics-LoRA** は、全パラメータ更新（Full-FT）と比較して、更新対象を約 **24% 削減**（約65万パラメータ削減）できています。
    *   Encoder-Frozen だけでは削減幅が少ないのは、TD-MPCにおいてエンコーダ（CNN）よりもDynamics/Reward/PiモデルのMLP部分がパラメータの多くを占めているためです。LoRAをDynamics/Rewardモデルに適用することで、大幅な削減が実現されています。

2.  **推論・学習時間**
    *   **推論時間**: LoRAは元の重みに加えて低ランク行列（A, B）の計算パスが追加されるため、わずかに（約15ms）増加しています。
    *   **学習時間**: 更新パラメータ数が少ないため、バックプロパゲーション時の勾配計算コストが抑えられ、Dynamics-LoRAが最も高速（1ステップあたり約77ms）という結果になりました。

3.  **メモリ使用量**
    *   メモリ使用量は各手法でほぼ同等（約1.2GB）です。これは、パラメータをフリーズしてもモデル自体はメモリ上に保持されること、およびリプレイバッファや中間活性化値が占める割合が大きいためです。

### 結論
**Dynamics-LoRA** は、推論時間に若干のオーバーヘッドがあるものの、**学習時の計算効率が最も高く、更新パラメータ数を大幅に抑えながらファインチューニングが可能**であることが確認できました。

※テスト実行にあたっては、実行中の学習データへの影響を避けるため、一時的な実験名（`profile_tmp`）を使用し、ビデオやモデルの保存を無効化して安全に実施しました。
